...
export const storageService = {
  ...
  addShortLink(shortLink: Omit<ShortLink, 'id' | 'createdAt' | 'updatedAt'>): ShortLink {
    ...
    // 检查是否已经存在相同原始链接的记录
    const existingIndex = shortLinks.findIndex(link => link.originalUrl === shortLink.originalUrl)
    ...
    if (existingIndex !== -1) {
      ...
      // 检查是否已经存在相同短码的记录
      const existingShortCodeIndex = shortLinks.findIndex(link => link.shortCode === shortLink.shortCode)
      if (existingShortCodeIndex !== -1) {
        // 如果存在相同短码的记录，更新它
        shortLinks[existingShortCodeIndex] = {
          ...shortLinks[existingShortCodeIndex],
          ...shortLink,
          updatedAt: new Date().toLocaleString('zh-CN', { hour12: false })
        }
        localStorage.setItem(STORAGE_KEY, JSON.stringify(shortLinks))
        return shortLinks[existingShortCodeIndex]
      } else {
        // 如果不存在相同原始链接和短码的记录，添加新的记录
